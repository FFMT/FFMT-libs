...
/**
 * This class is useable to replace EntityMob (e.g EntityMyEntity extends
...
public abstract class FFMTEntityMobCustomized extends EntityMob
{
	...
	public FFMTEntityMobCustomized(World world)
	{
		super(world);
		entityPhases = 0;
		bossStrength = 8;
	...
	//TODO rewrite entity code 
	/*
	public boolean attackEntityMob(Entity entity)
	{
		...
		if(flag)
		{
			if(j > 0)
			{
				entity.addVelocity((double)(-MathHelper.sin(this.rotationYaw * (float)Math.PI / 180.0F) * (float)j * 0.5F), 0.1D, (double)(MathHelper.cos(this.rotationYaw * (float)Math.PI / 180.0F) * (float)j * 0.5F));
				this.motionX *= 0.6D;
				this.motionZ *= 0.6D;
			}
